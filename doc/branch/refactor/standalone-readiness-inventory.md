...
|---|---|---|---|---|
| **Must** | Project-local NuGet restore is required before standalone build | `.vcxproj` imports `packages\Microsoft.Direct3D.*` and `packages\WinPixEventRuntime.*`; `packages/` is ignored and should be restored locally. | A fresh standalone checkout will not build until packages are restored. | Run `nuget restore packages.config -PackagesDirectory packages` or equivalent Visual Studio restore into project-local `packages/`. |
...
1. **Document standalone restore flow** — Add a short README or build note for `nuget restore packages.config -PackagesDirectory packages` and vcpkg manifest restore.
