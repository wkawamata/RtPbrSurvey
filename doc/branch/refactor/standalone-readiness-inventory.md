...
The script uses `tools\nuget.exe` when present, otherwise `nuget.exe` from `PATH`. It restores into the project-local `packages/` directory expected by the `.vcxproj`.
