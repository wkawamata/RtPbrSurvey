...
|---|---|---|---|---|
| **Should** | Document project-local NuGet restore before standalone build | `.vcxproj` imports `packages\Microsoft.Direct3D.*` and `packages\WinPixEventRuntime.*`; `packages/` is ignored and should be restored locally. | This is a normal fresh-checkout build prerequisite, not an extraction blocker. | Document `nuget restore packages.config -PackagesDirectory packages` or equivalent Visual Studio restore into project-local `packages/`. |
