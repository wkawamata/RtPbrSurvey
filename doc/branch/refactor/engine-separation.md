...
`D3D12HelloTexture` の巨大な責務を `RtPbrSurveyEngine` に丸ごと移すと、名前が変わっただけで構造は Sample のままになります。
...
* App 固有の UI と scene selection は `App/RtPbrSurveyApp` に寄せる
...
* `D3D12HelloTexture` を丸ごと `RtPbrSurveyEngine` に rename する
...
最終的には、`RtPbrSurvey` を DirectX-Graphics-Samples の他サンプルから分け、独立した repository にする。

ただし、単純なファイルコピーではなく、Git の履歴を残したまま切り出す。
基本方針は、切り出し時に `git filter-repo` で `RtPbrSurvey` の path だけを残し、その path を repository root に rename すること。
...
git filter-repo `
  --path Samples/Desktop/D3D12HelloWorld/src/RtPbrSurvey/ `
  --path-rename Samples/Desktop/D3D12HelloWorld/src/RtPbrSurvey/:
```

この方法なら、`RtPbrSurvey` 配下に対して積んだ commit 履歴は standalone repository 側にも残る。

### Standalone 化までに守ること

* できるだけ必要な source / shader / asset / doc を `RtPbrSurvey` 配下へ集約する。
...
* `RtPbrSurvey` 配下で意味のある小さな commit を積む。
* 生成物や log は commit しない。

### 切り出し前に確認すること

* project file が `RtPbrSurvey` の外側にある source / props / targets / assets を参照していないか。
...
まずは現在の repository 内で、`RtPbrSurvey` 配下の所有境界を整理し、切り出しやすい状態を作る。
...
* `RtPbrSurvey.vcxproj` から `<ClInclude>` エントリを削除。
* `RtPbrSurvey.vcxproj.filters` からフィルターエントリを削除。
