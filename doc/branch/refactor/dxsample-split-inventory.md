...
ただし `SampleApp` への委譲が進んでおり、多くのレンダリング関連責務は既に `RtPbrSurveyEngine` / `GraphicsDevice` 側に移動済みです。
...
- D3D12 debug layer 管理は `SampleApp` (ID3D12InfoQueue) と `RtPbrSurveyEngine` が担当
