...
HDR diagnostic schema version 8 retains the ROI-center `referenceSurfaceSample` introduced in version 3 and the weighted estimate/moments signals introduced in version 4. It records the configured temporal history weight, policy identifier, and whether the variance-guided temporal experiment is enabled. It also reads visible PBR parameters so the report can summarize the next accepted same-pixel policy weight with the material confidence gate applied. This policy-weight diagnostic does not apply motion reprojection and is therefore scoped to the fixed-camera ROI workflow. The report exposes regular linear-HDR temporal statistics for the resolved weighted estimate and reports mean first moment, mean second moment, mean estimated variance, and maximum estimated variance for the moments resource. These moments describe the current temporal estimator history; they are neither a physical ground truth nor the variance of a long-run independent reference integration. A 1x1 ROI may use the exact center-surface condition with the independent uniform-hemisphere integrator in `Tests/HybridReflection`. Wider ROI statistics must not be compared with the single center-pixel reference as if their surface conditions were identical.
