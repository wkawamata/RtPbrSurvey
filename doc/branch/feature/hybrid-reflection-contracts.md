...
The first default-off variance-guided temporal experiment derives prior relative variance as `saturate(max(M2 - M1^2, 0) / max(M2, 1e-6))` and moves the weighted-estimator history weight from the configured base toward `0.98`. It does not alter legacy unweighted resolved radiance. Paired controlled-scene measurements rejected this formula for promotion: it reduced roughness `1.0` variance but changed the 256-frame mean, and roughness `0.35` temporal variance increased. The implementation remains diagnostic and default-off; the formula is not a recommended production policy.
...
HDR diagnostic schema version 5 retains the ROI-center `referenceSurfaceSample` introduced in version 3 and the weighted estimate/moments signals introduced in version 4. It also records the configured temporal history weight and whether the variance-guided temporal experiment is enabled. The report exposes regular linear-HDR temporal statistics for the resolved weighted estimate and reports mean first moment, mean second moment, mean estimated variance, and maximum estimated variance for the moments resource. These moments describe the current temporal estimator history; they are neither a physical ground truth nor the variance of a long-run independent reference integration. A 1x1 ROI may use the exact center-surface condition with the independent uniform-hemisphere integrator in `Tests/HybridReflection`. Wider ROI statistics must not be compared with the single center-pixel reference as if their surface conditions were identical.
