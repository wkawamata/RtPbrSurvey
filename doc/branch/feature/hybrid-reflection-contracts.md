...
| `ReflectionSpecularConfidence` | `TemporalReflectionPass` MRT target 5, ping-pong pair | `R16_FLOAT`; scalar persistent confidence in `[0, 1]`. Rejected/reset history writes `0`; accepted history updates from reprojected confidence and prior moments. | Experimental policy metadata | `Specular Confidence` debug view, HDR diagnostics (introduced in schema v9 and retained in current schema), and default-off confidence-guided history weighting. It is not radiance. |
...
HDR diagnostic schema version 9 introduced the ROI-center `referenceSurfaceSample`, weighted estimate, moments, visible PBR inputs, and motion-reprojected confidence/effective-weight distributions. Those fields remain in current schema v15. Unlike the schema v8 same-pixel prediction, the current report records shader output after history acceptance/rejection and confidence update. The report also exposes regular linear-HDR temporal statistics for the resolved weighted estimate and moments summaries. These signals describe the current temporal estimator history; they are neither a physical ground truth nor the variance of a long-run independent reference integration. A 1x1 ROI may use the exact center-surface condition with the independent uniform-hemisphere integrator in `Tests/HybridReflection`. Wider ROI statistics must not be compared with the single center-pixel reference as if their surface conditions were identical.
...
Detailed Raster/Hybrid/future-PT/future-RR comparison rules live in [hybrid-reflection-comparison-contracts.md](hybrid-reflection-comparison-contracts.md). In summary, every comparison must name its signal boundary: evaluated and resolved radiance are unweighted reflection signals at different temporal stages, while Final Lit is a weighted scene-color boundary and presentation output is display-referred. Current PT and DLSS RR results remain unclaimed; this resource contract does not connect Streamline SDK types or an RR backend.
