...
bool SetSceneNodeWorldTransform(SceneDocument& document,
                                ...
                                std::string* error)
{
    SceneGraphEvaluation evaluation;
    if (!EvaluateSceneGraph(document, evaluation, error))
    {
        return false;
    }
    ...
    for (size_t index = 0; index < document.nodes.size(); ++index)
    {
        ...
        {
            nodeIndex = index;
            if (document.nodes[index].parentId.has_value())
            {
                for (size_t parent = 0; parent < document.nodes.size(); ++parent)
                {
                    if (document.nodes[parent].id == *document.nodes[index].parentId)
                    {
                        parentIndex = parent;
                        break;
                    }
    ...
    if (parentIndex != document.nodes.size())
    {
        DirectX::XMVECTOR determinant;
        const DirectX::XMMATRIX inverseParent = DirectX::XMMatrixInverse(
            &determinant, DirectX::XMLoadFloat4x4(&evaluation.worldTransforms[parentIndex]));
        if (std::abs(DirectX::XMVectorGetX(determinant)) <= 0.000001f)
        {
            if (error != nullptr)
            {
                *error = "Scene graph parent transform is not invertible.";
            }
            return false;
        }
        localTransform = localTransform * inverseParent;
    }
    ...
    if (!DirectX::XMMatrixDecompose(&scale, &rotation, &translation, localTransform))
    {
        if (error != nullptr)
        {
            *error = "Scene graph transform cannot be represented as TRS.";
        }
        return false;
    }
    SceneNode& node = document.nodes[nodeIndex];
    DirectX::XMFLOAT3 storedTranslation;
    DirectX::XMFLOAT4 storedRotation;
    DirectX::XMFLOAT3 storedScale;
    DirectX::XMStoreFloat3(&storedTranslation, translation);
    DirectX::XMStoreFloat4(&storedRotation, rotation);
    DirectX::XMStoreFloat3(&storedScale, scale);
    node.transform.translation = {storedTranslation.x, storedTranslation.y, storedTranslation.z};
    node.transform.rotation = {storedRotation.x, storedRotation.y, storedRotation.z, storedRotation.w};
    node.transform.scale = {storedScale.x, storedScale.y, storedScale.z};
    return true;
}
